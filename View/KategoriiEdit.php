...
function KategoriiEdit(){
	...
	function Paint(){
		?>
		<a href='?view=KategoriiView'>К списку</a>
		<table border="1">
		<form method="POST" enctype="multipart/form-data" action="?view=KategoriiEdit&action=updateKategoriiAction&id=<?=$this->id?>">

			<tr><td>ID</td><td><input type="text" name="id" value="<?php echo($this->category['id']);?>"></td></tr>
			<tr><td>parent</td><td><input type="text" name="parent" value="<?php echo($this->category['parent']);?>"></td></tr>
